...
def task_instruction(state: dict) -> str:
    ...
    if ctx == "playing_blind":
        return (
            ...
            "and say it may fall short. Free card-cycling: a played hand may include up to 5 cards; "
            "cards beyond the scoring hand do NOT score but still LEAVE your hand and get replaced by "
            "fresh draws -- a free cycle that costs no discard. So when you play a small hand (e.g. a "
            "pair) and still have hands left, usually ALSO add your most useless cards up to 5 total to "
            "dig for a bigger hand (check 'deck_remaining' for what's still drawable); skip this on the "
            "last hand or when the extra cards are worth keeping (a flush/straight in progress, or high "
            "cards to hold for held-in-hand jokers). Exploit each joker's 'desc': jokers that scale off cards still HELD in hand "
            ...
            "FACE-DOWN (a boss blind): you cannot SEE its exact rank/suit and must never claim to. "
            "But like a deck-tracking player you MAY estimate what it is likely to be from 'deck' "
            "(the full deck) minus the cards you can already see, and factor that probability in -- "
            "hedged, never as certain (e.g. 'the hidden card is probably low'). If the whole hand is "
            "hidden, usually just play a hand to see rather than discard blindly."
        )
